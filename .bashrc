...
pyvers() {
    ...
    if [ ! -z "$(parse_git_branch)" ]; then
        local pypresent="$(find . -type f -name '*.py')"
    else pypresent=""
    fi

    if [ ! -z "$pypresent" ]; then
        echo -en " 🐍 $(python -V 2>&1| sed -e "s/Python//g" | xargs) "
    fi
}

if [ "$color_prompt" = yes ]; then

 PS1='${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]:\[\033[01;36m\]\w\[\033[01;35m\]$(parse_git_branch) \[\033[01;35;107m\]$(origin_dist)\[\033[01;93;49m\]$(pyvers)\n\[\033[00m\]💻 '
else
